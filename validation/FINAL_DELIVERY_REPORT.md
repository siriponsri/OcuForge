...
champion. R1-P0 acquisition/runtime checks completed but are `BLOCKED` by two exact MMRDR duplicate-content groups
crossing the released train/test split; the official split remains unchanged. Exact findings, asset revisions, overlap
limits, and the future download manifest are recorded in the [P0 record](../docs/R1_P0_ACQUISITION_PREFLIGHT.json) and
...
No R1 candidate was trained and no R1 benchmark was executed. The owner-authorized public RunPod acquisition/runtime
validation completed with compact redacted evidence kept under ignored `local-state/`; no hospital/private data,
credentials, or private derived artifacts were inspected or uploaded.
