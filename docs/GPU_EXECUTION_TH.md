...
`READY_NOT_EXECUTED`. This document does not authorize training or provider provisioning. R1-P0 may acquire only the
frozen public inputs and approved assets under its contract.
...
After R0 and R1-P0 pass, run one candidate at a time from the registry: C0, stop/review; C1, stop/review; C2, stop/review.
No candidate may train before R1-P0 passes.
