...
1. `git pull` and execute R1-P0: verify archive/file hashes, inventories, schema/split, preprocessing, gated access,
   model loading, and storage/runtime readiness.
2. If R1-P0 passes, run R1 one candidate at a time: C0, C1, C2. Stop after each run for human review.
