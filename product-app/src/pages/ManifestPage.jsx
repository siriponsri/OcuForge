...
export function ManifestPage({ cases, onRebuild }) {
  ...
  const effectiveEligibility = useMemo(() => new Map(rows.map((row) => [row.case_id, row.training_eligibility])), [rows]);
  const counts = Object.fromEntries(ELIGIBILITY.map((value) => [value, rows.filter((row) => row.training_eligibility === value).length]));
  const filtered = useMemo(() => filter === "ALL" ? cases : cases.filter((item) => effectiveEligibility.get(item.id) === filter), [cases, effectiveEligibility, filter]);
