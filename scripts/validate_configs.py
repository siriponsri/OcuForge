...
def main():
    ...
    for p in (ROOT / "eyes-detected-models/configs/datasets").glob("*.yaml"):
        DatasetManifest.model_validate(yaml.safe_load(p.read_text(encoding="utf-8")))
        count += 1
    model = yaml.safe_load(
        (ROOT / "eyes-detected-models/configs/models/attention_coral.yaml").read_text(encoding="utf-8")
    )
